---
name: goal-hooks
description: Author pre/post-iteration hooks for a goal session. Use when the user asks to add research fetching, Slack/webhook notifications, persistent learnings, auto-tagging, anti-thrash intervention, idea rotation, or any side effect around iterations.
---

# goal-hooks

Optional scripts that run at iteration boundaries in a goal session. Two hooks, both transparent to the loop-running agent — their effect is a file on disk or a steer message.

```
goal.hooks/
  before.sh    # fires before each iteration (prospective)
  after.sh     # fires after each log_goal (retrospective)
```
...
| `session.goal`            | The session name set by `init_goal`.                          |
...
`goal.hooks/**` survives the auto-revert, like all paths matching `goal.*`.
...
Each example is a complete, self-contained script with named constants, short helper functions, guard clauses, and intention-revealing names. Read the header comment for its purpose, copy to `goal.hooks/<stage>.sh`, adapt.

---

## Steps to add a hook

1. **Understand the session.** Read `goal.md` for the objective and metric; glance at `goal.sh` for the workload. Your hook should complement the loop, not duplicate it.
...
4. **Copy, adapt, mark executable.**
   ...
   ```bash
   mkdir -p goal.hooks
   cp "<skill-dir>/examples/before/external-search.sh" goal.hooks/before.sh
   # ... adapt the script ...
   chmod +x goal.hooks/before.sh
   ```
...
6. **Commit the hook** alongside other session files. It's preserved across reverts because the path matches `goal.*`.
...
- **No environment variables.** Everything is on stdin; extract `cwd` (and anything else) with `jq`. There is no goal-specific environment variable.
