...
name: goal-finalize
context: fork
description: Finalize a goal session into clean, reviewable branches. Use when asked to "finalize goal", "clean up experiments", or "prepare goal for review".
---

# Finalize Goal

Turn a noisy goal branch into clean, independent branches — one per logical change, each starting from the merge-base.

## Step 1 — Analyze and Propose Groups

1. Read `goal.jsonl`. Filter to **kept** experiments only.
2. Read `goal.md` for context.
...
The script creates one branch per group from the merge-base, verifies the union matches the original branch, and prints a summary with all branches, cleanup commands, and any ideas from `goal.ideas.md`.
