...
setup_repo() {
  ...
  # Goal branch
  git checkout -b goal/test-session --quiet

  # Session files (should not end up in branches)
  echo '{"type":"config","metricName":"ms","metricUnit":"ms","bestDirection":"lower"}' > goal.jsonl
  echo "# Goal session" > goal.md
  echo "#!/bin/bash" > goal.sh
  echo "- try X" > goal.ideas.md
...
EOF
  ...
  git rev-parse "goal/test/01-optimize-a" >/dev/null 2>&1 || { fail_test "basic two groups" "Branch 01 not created"; cleanup_repo "$REPO"; return; }
  git rev-parse "goal/test/02-optimize-b" >/dev/null 2>&1 || { fail_test "basic two groups" "Branch 02 not created"; cleanup_repo "$REPO"; return; }

  # Check each branch starts from BASE
  local BASE_OF_A BASE_OF_B
  BASE_OF_A=$(git merge-base "goal/test/01-optimize-a" main)
  BASE_OF_B=$(git merge-base "goal/test/02-optimize-b" main)
  ...
  COUNT_A=$(git rev-list --count "$BASE"..goal/test/01-optimize-a)
  COUNT_B=$(git rev-list --count "$BASE"..goal/test/02-optimize-b)
  ...
  A_CONTENT=$(git show goal/test/01-optimize-a:file_a.txt)
  B_CONTENT=$(git show goal/test/02-optimize-b:file_b.txt)
  ...
  A_FILEB=$(git show goal/test/01-optimize-a:file_b.txt)
  B_FILEA=$(git show goal/test/02-optimize-b:file_a.txt)
...
EOF
  ...
  for f in goal.jsonl goal.sh goal.md goal.ideas.md; do
    if git show "goal/test/01-all":"$f" &>/dev/null 2>&1; then
...
EOF
  ...
  [ "$CURRENT" = "goal/test-session" ] || { fail_test "rollback on failure" "Not on original branch: $CURRENT"; cleanup_repo "$REPO"; return; }

  # No leftover branches
  if git rev-parse "goal/test/01-good" >/dev/null 2>&1; then
...
EOF
  ...
  echo "$OUTPUT" | grep -q "goal.ideas.md" || { fail_test "summary output" "Missing ideas in output"; rm -f "$GROUPS_JSON"; cleanup_repo "$REPO"; return; }

  # No summary file should be written to disk
  [ ! -f "goal-finalize-summary.md" ] || { fail_test "summary output" "Summary file written to disk — should only print"; rm -f "$GROUPS_JSON"; cleanup_repo "$REPO"; return; }
...
test_branch_collision() {
  ...
  git branch "goal/test/01-optimize-a" "$BASE"
...
EOF
  ...
  git rev-parse "goal/test/01-all" >/dev/null 2>&1 || { fail_test "single group" "Branch not created"; cleanup_repo "$REPO"; return; }

  # Should have both file changes
  local A B
  A=$(git show goal/test/01-all:file_a.txt)
  B=$(git show goal/test/01-all:file_b.txt)
...
test_nested_session_artifacts() {
  ...
  git checkout -b goal/nested-test --quiet

  # Session files in a subdirectory (like world's libraries/javascript/polaris/)
  echo '{"type":"config"}' > libs/polaris/goal.jsonl
  echo "# session" > libs/polaris/goal.md
  echo "#!/bin/bash" > libs/polaris/goal.sh
  echo "- idea" > libs/polaris/goal.ideas.md
  echo "#!/bin/bash" > libs/polaris/goal.checks.sh
...
EOF
  ...
  # Branch should only have component.ts, not any goal.* files
  local BRANCH="goal/test/01-optimize"
...
EOF
  ...
  git rev-parse "goal/test/01-optimize-a" >/dev/null 2>&1 || { fail_test "verify failure leaves branches" "Branch was deleted"; cleanup_repo "$REPO"; return; }

  # Should be back on original branch
  local CURRENT
  CURRENT=$(git branch --show-current 2>/dev/null || echo "")
  [ "$CURRENT" = "goal/test-session" ] || { fail_test "verify failure leaves branches" "Not on original branch: $CURRENT"; cleanup_repo "$REPO"; return; }
...
EOF
  ...
  local CURRENT
  CURRENT=$(git branch --show-current 2>/dev/null || echo "")
  [ "$CURRENT" = "goal/test-session" ] || { fail_test "stash pop on rollback" "Not on original branch: $CURRENT"; cleanup_repo "$REPO"; return; }
...
test_three_groups() {
  ...
  git checkout -b goal/three-test --quiet
...
EOF
  ...
  git rev-parse "goal/three/01-a" >/dev/null 2>&1 || { fail_test "three groups" "Branch 01 missing"; cleanup_repo "$REPO"; return; }
  git rev-parse "goal/three/02-b" >/dev/null 2>&1 || { fail_test "three groups" "Branch 02 missing"; cleanup_repo "$REPO"; return; }
  git rev-parse "goal/three/03-c" >/dev/null 2>&1 || { fail_test "three groups" "Branch 03 missing"; cleanup_repo "$REPO"; return; }

  # Each branch is independent — 1 commit from base
  for b in goal/three/01-a goal/three/02-b goal/three/03-c; do
    local C
  ...
  [ "$(git show goal/three/01-a:file_a.txt)" = "optimized_a" ] || { fail_test "three groups" "01-a: wrong file_a"; cleanup_repo "$REPO"; return; }
  [ "$(git show goal/three/01-a:file_b.txt)" = "original_b" ] || { fail_test "three groups" "01-a: has file_b changes"; cleanup_repo "$REPO"; return; }
  [ "$(git show goal/three/02-b:file_b.txt)" = "optimized_b" ] || { fail_test "three groups" "02-b: wrong file_b"; cleanup_repo "$REPO"; return; }
  [ "$(git show goal/three/02-b:file_a.txt)" = "original_a" ] || { fail_test "three groups" "02-b: has file_a changes"; cleanup_repo "$REPO"; return; }
  [ "$(git show goal/three/03-c:file_c.txt)" = "optimized_c" ] || { fail_test "three groups" "03-c: wrong file_c"; cleanup_repo "$REPO"; return; }
